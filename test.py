...
# m = cp.Model()
# b = cp.boolvar(name="b")
# x = cp.intvar(1, 10000, shape=500, name="x")
# y = cp.intvar(1, 10000, shape=500, name="y")
#
# m += (x[0] == 1)
# m += cp.AllDifferent(x)
# m += b.implies(x[1] + x[2] > 5000)
# m += cp.AllDifferent(y)
# m += b.implies(y[1] + y[2] > 5000)
# m += (cp.sum(x) + cp.sum(y) > 200000)
#
# for i in range(500):
#     m += (x[i] + y[i] <= 10000)
#     m += (x[i] * y[i] >= 1000)
...
# for i in range(497):
#     m += (x[i] + y[i+3] * b <= 8000)
#     m += (y[i+3] - x[i+1] * b >= -3000)
#
# m.maximize(cp.sum(x) + cp.sum(y) + 100 * b)
#
# tunables = {
...
# # solver = "ortools"
# # tuner = ParameterTuner(solver, m, tunables, defaults)
# # best_params = tuner.tune(time_limit=20, max_tries=10, HPO = "Bayesian")#max_tries=100 , , time_limit=60 HPO= ,"Hamming", "Bayesian", "Grid"
# # best_runtime = tuner.best_runtime
# #
# # print(best_params)
# # print(best_runtime)
